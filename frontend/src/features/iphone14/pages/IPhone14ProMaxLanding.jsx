...
const FRAMES = [
  ...
  },
  {
    id: 'engine',
    label: 'The Engine',
    bg: engineImg, // Using placeholder for A16 chip
    accentColor: '#E87B2C',
  }
];
...
function ArrivalOverlay() {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-end pb-[12%] z-20 pointer-events-none">
      ...
      >
        <span className="lens-flare-text">The ultimate Pro.</span>
      </motion.p>
    </div>
  );
}
    ...
    <div className="absolute inset-0 z-20 bg-black overflow-hidden pointer-events-auto" style={{ perspective: "1200px" }}>
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ backfaceVisibility: "hidden" }}>
          <img src={khungImg} alt="Outside Chassis" className="w-full h-full object-contain" />
        </div>

        {/* Back Face (Inside) - rotated 180deg so it faces the back */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ transform: "rotateY(180deg)", backfaceVisibility: "hidden" }}>
          <img src={khungInsideImg} alt="Inside Chassis" className="w-full h-full object-contain" />
        </div>
      </motion.div>
...
function EngineOverlay() {
  const stats = [
    { value: '16', label: 'Bionic' },
    { value: '4nm', label: 'Tech' },
  ];
  return (
...
export default function IPhone14ProMaxLanding() {
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      trigger: section,
      start: 'top top',
      end: `+=800%`, // Adjusted for 4 frames, faster scroll
      pin: true,
      scrub: 0.8, // reduced scrub smoothing for faster response
      anticipatePin: 1,
      onUpdate: (self) => {
        ...
        let idx = 0;
        if (p < 0.20) idx = 0;
        else if (p < 0.35) idx = 1;
        else if (p < 0.70) idx = 2;
        else idx = 3;
        setActiveFrame(idx);
      },
    });
