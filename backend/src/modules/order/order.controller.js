...
const stripe = require('stripe')(process.env.STRIPE_SECRET_KEY || 'dummy_key_to_prevent_crash'); // Sẽ báo lỗi nếu chưa cấu hình KEY nhưng không làm crash toàn cục
...
/**
 * Tạo Đơn Hàng (Sẽ được gọi tự động bằng Stripe Webhook, nhưng tạm thời gọi thủ công)
 */
  ...
  try {
    ...
    const newOrder = await prisma.order.create({
      data: {
        ...
        cancelReason: notes, // map notes tạm thời
...
/**
 * VNPAY Webhook (IPN) - Source of Truth
 */
const vnpayWebhook = async (req, res) => {
  try {
    ...
    // 2. TODO: Khởi tạo và xác thực chữ ký (Signature HMAC-SHA512)
    ...
    const isValidSignature = true; // TODO: Thay bằng signed === secureHash
    ...
    if (isValidSignature) {
      ...
      // Idempotency check: Tránh update lại nếu đã thành công
...
const switchToCOD = async (req, res) => {
  try {
    ...
    // Chỉ cho phép chuyển sang COD nếu chưa thanh toán
    if (order.paymentStatus === 'PAID') {
